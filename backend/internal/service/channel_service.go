...
func matchingPlatforms(groupPlatform string) []string {
	if groupPlatform == PlatformComposite {
		return []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformLeo, PlatformOpenAIMedia, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo}
	}
	...
	if p.BillingMode == BillingModeVideo {
		...
		if p.PerRequestPrice == nil && len(p.Intervals) == 0 {
			return infraerrors.BadRequest(
				...
				"per-request price or intervals required for video billing mode",
			)
		}
		return nil
	}
