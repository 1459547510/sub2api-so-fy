...
func TestSchedulerGroupLifecycleActiveReopensAndRebuildsAllCurrentBuckets(t *testing.T) {
	...
	require.Len(t, reopenHeld, len(expectedGroupLifecycleBuckets(groupID)))
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	lockTTLs, unlockCalls := cache.lockStats()
	require.Len(t, lockTTLs, len(expectedGroupLifecycleBuckets(groupID)))
	for _, ttl := range lockTTLs {
		require.Equal(t, 30*time.Second, ttl)
	}
	require.Equal(t, len(expectedGroupLifecycleBuckets(groupID)), unlockCalls)
	requireLifecycleSeen(t, seen, groupID)
}
...
func TestSchedulerGroupLifecycleEpochPreventsABA(t *testing.T) {
	...
	require.Greater(t, allTokens[len(expectedGroupLifecycleBuckets(groupID))].Epoch, firstActiveTokens[0].Epoch)
