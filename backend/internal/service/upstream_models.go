...
type UpstreamModelCatalog struct {
	...
	ModelDetails []UpstreamModel                  `json:"model_details,omitempty"`
...
// display names and media kinds when the upstream provides them.
func (s *AccountTestService) FetchUpstreamSupportedModelDetails(ctx context.Context, account *Account) ([]UpstreamModel, error) {
	...
	return upstreamModelDetailsFromList(account, models, body)
}
...
func (s *AccountTestService) SyncUpstreamModelCatalog(ctx context.Context, account *Account) (*UpstreamModelCatalog, error) {
	...
	if details, detailsErr := upstreamModelDetailsFromList(account, models, body); detailsErr == nil {
		catalog.ModelDetails = details
	}
	if len(body) > 0 {
...
func (s *AccountTestService) fetchUpstreamModelList(ctx context.Context, account *Account) ([]string, []byte, error) {
	...
	if account.IsMediaAPIAccount() {
		details, detailsErr := upstreamModelDetailsFromList(account, models, body)
		if detailsErr != nil {
			return nil, nil, newUpstreamModelSyncUpstreamError("Upstream model list response was not valid JSON", detailsErr)
		}
		models = upstreamModelIDs(details)
	}
...
func upstreamModelDetailsFromList(account *Account, models []string, body []byte) ([]UpstreamModel, error) {
	var details []UpstreamModel
	var err error
	if len(body) > 0 {
	...
	publicModels := details[:0]
	seenPublicIDs := make(map[string]struct{}, len(details))
