...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	if endpoint == service.GrokMediaEndpointImagesGenerations || endpoint == service.GrokMediaEndpointImagesEdits {
		if err := requestInfo.ValidateImageCount(); err != nil {
			h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", err.Error())
			return
		}
	}
	...
	if (endpoint == service.GrokMediaEndpointImagesGenerations || endpoint == service.GrokMediaEndpointImagesEdits) && requestInfo.N > 1 {
		if h.handleGrokImagesNSplit(
			c,
			apiKey,
			subject,
			subscription,
	...
	for {
		...
		if endpoint == service.SeedanceEndpointStatus {
			if billResult := prepareSeedanceCompletionBilling(requestCtx, h, apiKey, subject, requestID, result); billResult != nil {
				recordGrokMediaUsage(requestCtx, c, h, reqLog, apiKey, subject, subscription, account, billResult, billResult.Model, body, requestID)
			}
			...
			if billResult := prepareGrokVideoCompletionBilling(requestCtx, h, reqLog, apiKey, subject, taskID, result); billResult != nil {
				recordGrokMediaUsage(requestCtx, c, h, reqLog, apiKey, subject, subscription, account, billResult, billResult.Model, body, taskID)
			}
		} else if shouldRecordGrokMediaUsage(endpoint, requestModel, result) {
			recordGrokMediaUsage(requestCtx, c, h, reqLog, apiKey, subject, subscription, account, result, requestModel, body, requestID)
		}
...
func recordGrokMediaUsage(
	...
	h.submitOpenAIUsageRecordTask(usageParent, result, func(ctx context.Context) {
