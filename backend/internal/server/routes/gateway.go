...
func RegisterGatewayRoutes(
	...
	r.POST("/alpha/search", textBodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, func(c *gin.Context) {
		if rejectLeoUnsupported(c, "Alpha Search API") {
			return
		}
		h.OpenAIGateway.AlphaSearch(c)
	...
	r.POST("/videos/uploads", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, leoVideoOnly(h.OpenAIGateway.LeoVideoUpload))
	r.GET("/videos/jobs", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, leoVideoOnly(h.OpenAIGateway.LeoVideoJobs))
	r.GET("/videos/jobs/:job_id/content", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, leoVideoOnly(h.OpenAIGateway.LeoVideoJobContent))
	r.GET("/videos/jobs/:job_id", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, leoVideoOnly(h.OpenAIGateway.LeoVideoJob))
	r.DELETE("/videos/jobs/:job_id", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, leoVideoOnly(h.OpenAIGateway.CancelLeoVideoJob))
