...
// The VideoJobFunc type is an adapter to allow the use of ordinary
// function as VideoJob mutator.
type VideoJobFunc func(context.Context, *ent.VideoJobMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VideoJobFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.VideoJobMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VideoJobMutation", m)
}
