...
describe('locale key completeness', () => {
  ...
  it('keeps video workbench copy free of backend vendor names', () => {
    const vendorName = /Leonardo|LeoStudio|Leo\s*Studio|\bLeo\b|\bKrea\b|上游|upstream|\bprovider\b/i
    for (const [locale, messages] of Object.entries({ en, zh })) {
      const video = (messages as { video?: Record<string, unknown> }).video
      for (const key of ['tooManyAudioReferences', 'tooManyVideoReferences', 'tooManyImages']) {
        const text = video?.[key]
        expect(typeof text, `${locale}.video.${key}`).toBe('string')
        expect(String(text), `${locale}.video.${key}`).not.toMatch(vendorName)
      }
    }
  })
})
