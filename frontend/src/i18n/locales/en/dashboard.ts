  ...
  video: {
    ...
    apiDocs: {
      ...
      upload: {
         ...
         constraints: 'Use image for readable PNG, JPEG, or WebP up to 10 MiB. For reference video use the video field with a readable MP4/MOV container up to 100 MiB, no more than 3 per job. The browser checks local video decoding, but non-standard video dimensions or encoding may still be rejected during processing. For reference audio use the audio field with readable MP3 frames or RIFF/WAVE PCM 16/24-bit WAV up to 15 MiB and 2–30 seconds, no more than 1 per job, paired with an image or reference video. Use the returned address in the generation request.',
      },
      ...
      uploadMediaConstraints: 'Images must be readable PNG, JPEG, or WebP files up to 10 MiB. Videos must use an MP4/MOV filename and a readable ISO Base Media container, up to 100 MiB each, with no more than 3 per job. The public contract does not promise one fixed video duration, frame rate, codec, or dimension list; unusual files can still be rejected during processing. Audio must contain readable MP3 frames or RIFF/WAVE PCM 16/24-bit WAV, up to 15 MiB and 2 to 30 seconds, with no more than 1 per job; pair it with an image or reference video.',
      uploadMediaFormat: 'Reference-video flow: upload with the multipart video field, read media_url from the response, then place it at guidances.video_reference_base[].video.url with type: "UPLOADED". Reference video is available only to Seedance; Happy Horse and Grok are rejected during gateway validation.',
      createMediaDescription: 'Text, frames, image references, video references, and audio references use the same endpoint. Start and end frames may be submitted together, but reference images cannot be combined with either frame. Use no more than 3 video references and 1 audio reference; an audio reference must be paired with an image or reference video. Direct media URLs must be absolute HTTP(S), use a supported media extension or response type, and remain reachable while the job is queued. Data URLs, Base64, and multipart media inside the generation JSON are not accepted.',
      create: {
      ...
      fields: {
        ...
        yes: 'Yes',
        no: 'No',
        model: 'Model ID: seedance-2.0, seedance-2.0-fast, seedance-2.0-mini, happy-horse-1.1, or grok-imagine-1.5.',
        prompt: 'Describe the scene, action, camera, and visual style. Seedance/Grok allow up to 5,000 characters; Happy Horse allows up to 2,500.',
        resolution: 'The workbench filters resolutions by model: seedance-2.0 supports 480p, 720p, and 1080p; seedance-2.0-fast supports 480p and 720p; seedance-2.0-mini supports 480p and 720p; happy-horse-1.1 supports 720p and 1080p; grok-imagine-1.5 supports 400p, 544p, 720p, and 960p.',
        duration: 'Choose a whole number of seconds. Seedance models support 4 through 15 seconds, except seedance-2.0 at 1080p which supports 4 through 12. happy-horse-1.1 and grok-imagine-1.5 support 3 through 15 seconds. The default depends on the selected model.',
        aspectRatio: 'Aspect ratios are filtered by model and resolution. seedance-2.0-mini supports 16:9, 1:1, and 9:16. happy-horse-1.1 supports 16:9, 4:3, 1:1, 3:4, and 9:16. grok-imagine-1.5 supports 16:9 or 9:16 at 400p/720p, and 1:1 at 544p/960p.',
        ...
        endFrameUrl: 'An explicit absolute HTTP(S) end-frame URL. It may be submitted with a start frame, but not with reference images.',
         ...
         guidancesPublic: 'Reference media objects. Put uploaded media in image_reference[].image, video_reference_base[].video, or audio_reference[].audio; use the returned media_url and type: "UPLOADED". Limits are 9 images for Happy Horse, 4 for Seedance, 3 videos, and 1 audio; audio requires an image or video reference.',
        promptEnhance: 'happy-horse-1.1 only: AUTO, ON, or OFF. ON cannot be combined with a start frame.',
      },
      jobs: {
