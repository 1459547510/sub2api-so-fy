export default {
  ...
  video: {
    ...
    pricing: {
      ...
      title: 'Model pricing',
      description: 'Each model uses the administrator-configured unit price from that model’s channel pricing. Group per-model overrides win over a group-wide fallback.',
    ...
    apiDocs: {
      v2: {
        ...
        nav: { quick: 'Quick start', compatibility: 'V1 compatibility', discovery: 'Model discovery', pricing: 'Model prices', images: 'Image API', upload: 'Upload media', video: 'Video API', matrix: 'Per-model parameters', examples: 'Model examples', jobs: 'Jobs', status: 'Status and errors' },
        ...
        images: { title: 'OpenAI image endpoints', description: 'Image generation and editing follow the OpenAI Images request shape. OpenAI image models accept JSON URLs and multipart files; Leo image models accept reachable HTTP(S) reference URLs only and reject mask or multipart.', generations: 'Text-to-image. prompt is required. Leo image models can take image_urls or image_url on the same endpoint.', edits: 'Image editing. OpenAI models use images[].image_url and an optional mask; Leo image models rewrite images[].image_url to image_urls and forward /v1/images/generations. Mask and multipart are rejected.' },
        imageFields: { model: 'Model ID; it may be omitted for the server default, but an ID from /v1/models is recommended.', prompt: 'Generation or editing instruction.', n: 'Number of outputs, from 1 to 10.', size: 'Output size such as 1024x1024; use a supported value for the model.', responseFormat: 'url or b64_json; available values depend on the model.', quality: 'Optional quality tier; send it only when the model supports it.', imageUrls: 'Absolute HTTP(S) reference image URLs. Leo image models use this on /v1/images/generations and also accept a single image_url.', edits: 'OpenAI edits use images[].image_url and an optional mask. Leo image models accept images[].image_url only and rewrite it to image_urls. file_id is not part of the unified contract.' },
        ...
        matrix: {
          title: 'Per-model parameter matrix', description: 'Read /v1/models first to confirm the models available to the current Key, then choose a supported resolution, duration, aspect ratio, and reference input from the matching row. Seedance 2.0 / Fast / Mini now follow the Krea contract, not Leo’s per-resolution splits or the 1080p 12-second cap. Invalid combinations return 400 or 422.', model: 'Model', resolution: 'Resolution', duration: 'Duration', aspectRatio: 'Aspect ratio', references: 'Reference media',
          ...
          image: { resolution: 'Use size, model-specific', duration: 'Not applicable', aspectRatio: 'Determined by size or the model default', references: 'OpenAI image models use /v1/images/edits with images[] and an optional mask; Leo image models use image_urls, or rewrite edits images[].image_url to image_urls' },
          seedance20: {
            ...
            promptLimit: 'Krea does not publish a fixed limit; this gateway enforces 5000 characters',
        ...
        models: {
          seedance20: 'Krea Seedance 2.0 supports 480p/720p/1080p/4k for 4-15 seconds, default 5 seconds, at 16:9, 4:3, 1:1, 3:4, 9:16, or 21:9. Up to 9 image, 3 video, and 3 audio references are supported. This example is 4k text-to-video.',
          seedance20Fast: 'Krea Seedance 2.0 Fast supports 480p/720p for 4-15 seconds, default 5 seconds, with the same aspect ratios as standard. Up to 9 image, 3 video, and 3 audio references are supported.',
          seedance20Mini: 'Krea Seedance 2.0 Mini supports 480p/720p for 4-15 seconds, default 5 seconds, with the same aspect ratios as standard. It is no longer limited to 16:9/1:1/9:16. Up to 9 image, 3 video, and 3 audio references are supported.',
        },
