  ...
  video: {
    ...
    apiDocs: {
      ...
      v2: {
        ...
        images: { title: 'OpenAI image endpoints', description: 'Image generation and editing follow the OpenAI Images request shape. Some models accept JSON URLs and multipart files; others accept reachable HTTP(S) reference URLs only and reject mask or multipart. Check /v1/models and server validation for the current Key.', generations: 'Text-to-image. prompt is required. Reference images can be sent on the same endpoint as image_urls or image_url.', edits: 'Image editing. Use images[].image_url and an optional mask. If a model does not accept a separate edit request, the gateway rewrites images[].image_url to image_urls and sends /v1/images/generations. Those models reject mask and multipart.' },
        imageFields: { model: 'Model ID; it may be omitted for the server default, but an ID from /v1/models is recommended.', prompt: 'Generation or editing instruction.', n: 'Number of outputs, from 1 to 10.', size: 'Output size such as 1024x1024; use a supported value for the model.', responseFormat: 'url or b64_json; available values depend on the model.', quality: 'Optional quality tier; send it only when the model supports it.', imageUrls: 'Absolute HTTP(S) reference image URLs for /v1/images/generations; a single image_url is also accepted.', edits: 'Edits use images[].image_url and an optional mask. Some models accept images[].image_url only and the server rewrites it to image_urls. file_id is not part of the unified contract.' },
