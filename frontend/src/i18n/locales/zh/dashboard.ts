  ...
  video: {
    ...
    apiDocs: {
      ...
      upload: {
        ...
        constraints: '图片必须是可读取的 PNG、JPEG 或 WebP，单文件最大 10 MiB。参考视频使用 video 字段，必须是可读取的 MP4/MOV 容器，单文件最大 100 MiB，每个任务最多 3 个；页面会先检查本地视频能否解码，但非标准尺寸或编码仍可能在处理阶段被拒绝。参考音频使用 audio 字段，必须包含可读取的 MP3 帧，或 RIFF/WAVE PCM 16/24 位 WAV，单文件最大 15 MiB、时长 2 到 30 秒，每个任务最多 1 个，并且要搭配参考图或参考视频。',
      },
      uploadMediaTitle: '上传参考媒体',
      uploadMediaDescription: '上传图片、参考视频或参考音频后，使用返回的 media_url 创建视频任务。',
      uploadMediaConstraints: '图片支持 PNG、JPEG、WebP，单文件不超过 10 MiB；视频必须使用 MP4/MOV 文件名并且是可读取的 ISO Base Media 容器，单文件不超过 100 MiB，每个任务最多 3 个。公开接口没有承诺统一的视频时长、帧率、编码或尺寸白名单，非标准文件仍可能在处理阶段被拒绝；音频必须包含可读取的 MP3 帧，或 RIFF/WAVE PCM 16/24 位 WAV，单文件不超过 15 MiB，时长必须为 2 到 30 秒，每个任务最多 1 个，并且必须搭配参考图或参考视频。',
      uploadMediaFormat: '参考视频调用链：先用 multipart 的 video 字段上传，读取响应中的 media_url；再把它放入 guidances.video_reference_base[].video.url，并写入 type: "UPLOADED"。参考视频仅 Seedance 模型可用；Happy Horse 和 Grok 会在网关校验阶段拒绝。',
      createMediaDescription: '文本、首尾帧、参考图片、参考视频和参考音频使用同一个创建接口；首帧和尾帧可以同时提交，但参考图不能与任一首尾帧同时使用。参考视频最多 3 个，参考音频最多 1 个且必须搭配参考图或参考视频；直接填写媒体 URL 时必须是绝对 HTTP(S) 地址、扩展名或响应类型可识别，并且在排队期间保持可访问。创建 JSON 不接受 data URL、Base64 或 multipart 媒体。',
      create: {
        title: '创建视频任务',
      ...
      fields: {
        ...
        yes: '是',
        no: '否',
        model: '模型标识：seedance-2.0、seedance-2.0-fast、seedance-2.0-mini、happy-horse-1.1 或 grok-imagine-1.5。',
        prompt: '场景、动作、镜头和风格描述；Seedance/Grok 最多 5000 个字符，Happy Horse 最多 2500 个字符。',
        resolution: '工作台按模型过滤分辨率：seedance-2.0 支持 480p、720p、1080p；seedance-2.0-fast 支持 480p、720p；seedance-2.0-mini 支持 480p、720p；happy-horse-1.1 支持 720p、1080p；grok-imagine-1.5 支持 400p、544p、720p、960p。',
        duration: '视频秒数必须为整数；Seedance 模型支持 4 到 15 秒，但 seedance-2.0 的 1080p 仅支持 4 到 12 秒；happy-horse-1.1 和 grok-imagine-1.5 支持 3 到 15 秒。默认值按模型决定。',
        aspectRatio: '画面比例按模型和分辨率过滤；seedance-2.0-mini 支持 16:9、1:1、9:16；happy-horse-1.1 支持 16:9、4:3、1:1、3:4、9:16；grok-imagine-1.5 在 400p/720p 支持 16:9、9:16，在 544p/960p 仅支持 1:1。',
        ...
        endFrameUrl: '独立尾帧绝对 HTTP(S) 地址，可与首帧同时提交，但不能与参考图混用。',
         ...
         guidancesPublic: '参考媒体对象；图片使用 image_reference[].image，视频使用 video_reference_base[].video，音频使用 audio_reference[].audio；对象填写上传接口返回的 media_url 和 type: "UPLOADED"。Happy Horse 最多 9 张参考图，Seedance 最多 4 张；视频最多 3 个，音频最多 1 个；参考音频必须搭配参考图或参考视频。',
        promptEnhance: '仅 happy-horse-1.1 支持：AUTO、ON 或 OFF；ON 不能与首帧同时使用。',
      },
      jobs: {
        title: '查询与管理任务',
