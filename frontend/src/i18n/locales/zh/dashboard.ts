  ...
  video: {
    ...
    apiDocs: {
      ...
      v2: {
        ...
        images: { title: 'OpenAI 图片接口', description: '图片生成和编辑遵循 OpenAI Images 请求格式。部分模型支持 JSON URL 和 multipart；部分模型只接受可访问的 HTTP(S) 参考图 URL，不支持 mask 或 multipart。具体能力以 /v1/models 和服务端校验为准。', generations: '文生图。prompt 必填。需要参考图时，可在同一接口使用 image_urls 或 image_url。', edits: '图片编辑。可使用 images[].image_url 和可选 mask。若模型不接受独立编辑请求，网关会把 images[].image_url 转成 image_urls 并改走 /v1/images/generations；这类模型不接受 mask 或 multipart。' },
        imageFields: { model: '模型 ID；可省略使用服务端默认值，建议显式传入 /v1/models 返回的 ID。', prompt: '生成或编辑说明。', n: '生成数量，范围为 1-10。', size: '输出尺寸，例如 1024x1024；以模型支持范围为准。', responseFormat: 'url 或 b64_json；具体可用值按模型能力返回。', quality: '可选质量档位，只有模型支持时才发送。', imageUrls: '参考图绝对 HTTP(S) URL 数组；用于 /v1/images/generations，也接受单数 image_url。', edits: '编辑接口使用 images[].image_url 和可选 mask。部分模型只接受 images[].image_url，服务端会转成 image_urls。file_id 不在统一契约中。' },
    ...
    imageInputExclusive: '首帧和尾帧可以同时提交；参考图不能与任一首尾帧同时使用。',
     ...
     tooManyAudioReferences: '最多选择 1 个参考音频。',
