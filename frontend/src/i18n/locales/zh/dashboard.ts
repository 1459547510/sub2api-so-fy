export default {
  ...
  video: {
    ...
    apiDocs: {
      ...
      uploadMediaFormat: '参考视频调用链：先用 multipart 的 video 字段上传，读取响应中的 media_url；再把它放入 guidances.video_reference_base[].video.url，并写入 type: "UPLOADED"。参考视频仅 Seedance 模型可用；Happy Horse、Grok 和 LTX 会在网关校验阶段拒绝。',
      ...
      fields: {
        ...
        aspectRatio: '画面比例按模型和分辨率过滤。LTX 2.3 Pro/Fast 仅支持 16:9；其他模型比例见模型参数矩阵。',
        ...
        endFrameUrl: '独立尾帧绝对 HTTP(S) 地址，可与首帧同时提交，但不能与参考图混用。',
         ...
         guidancesPublic: '参考媒体对象；图片使用 image_reference[].image，视频使用 video_reference_base[].video，音频使用 audio_reference[].audio；对象填写上传接口返回的 media_url 和 type: "UPLOADED"。Happy Horse 最多 9 张参考图，Seedance 最多 4 张图、3 个视频、1 个音频；Grok 和 LTX 不支持媒体参考；参考音频必须搭配参考图或参考视频。',
        promptEnhance: 'happy-horse-1.1 和两个 LTX 2.3 模型支持 AUTO、ON 或 OFF；只有 Happy Horse 禁止 ON 与首帧同时使用。',
      },
      jobs: {
        title: '查询与管理任务',
