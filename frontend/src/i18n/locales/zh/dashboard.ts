  ...
  video: {
    ...
    apiDocs: {
      v2: {
        ...
        tocLabel: 'V2 文档目录',
        nav: {
          quick: '快速开始', compatibility: 'V1 兼容性', discovery: '模型发现', pricing: '模型价格', images: '图片接口', upload: '上传媒体', video: '视频接口', matrix: '参数规则', jobs: '查询与管理', status: '状态与错误',
        },
        ...
        images: { title: 'OpenAI 图片接口', description: '图片生成和编辑遵循 OpenAI Images 请求格式；支持 JSON URL 输入和 multipart 文件输入。', generations: '文生图。prompt 必填，model、n、size、response_format、quality 等字段按模型能力选择。', edits: '图片编辑。JSON 使用 images[].image_url 和可选 mask.image_url；multipart 使用 image[] 和可选 mask。' },
        imageFields: { model: '模型 ID；可省略使用服务端默认值，建议显式传入 /v1/models 返回的 ID。', prompt: '生成或编辑说明。', n: '生成数量，范围为 1-10。', size: '输出尺寸，例如 1024x1024；以模型支持范围为准。', responseFormat: 'url 或 b64_json；具体可用值按模型能力返回。', quality: '可选质量档位，只有模型支持时才发送。', edits: '仅编辑接口使用；图片 URL 必须可读取，file_id 不在统一契约中。' },
        ...
        matrix: {
          title: '参数规则', description: 'V2 不硬编码服务端模型清单。请先读取 /v1/models，再按模型能力选择参数；不支持的组合会返回 400 或 422。', model: '模型示例', resolution: '分辨率', duration: '时长', aspectRatio: '画面比例', references: '参考媒体',
          video: { resolution: '按模型能力', duration: '按模型能力，默认值由服务端决定', aspectRatio: '按模型能力', references: '首尾帧、图片、视频、音频按模型能力启用' },
          image: { resolution: '使用 size 字段，按模型能力', duration: '不适用', aspectRatio: '由 size 或模型默认值决定', references: '编辑接口使用 images[] 和可选 mask' },
        },
        examples: {
          videoRequest: '视频创建请求', acceptedResponse: '202 接收响应', modelList: '模型列表请求', imageGeneration: '图片生成请求', imageEdit: '图片编辑 JSON 请求', imageMultipart: '图片编辑 multipart 请求', imageResponse: '图片响应', upload: '媒体上传请求', uploadResponse: '媒体上传响应', uploadMedia: '上传视频和音频', videoStartFrame: '视频首帧请求', videoFramePair: '视频首尾帧请求', videoReferences: '视频参考图片请求', videoAudio: '视频参考音频请求', modelTitle: '统一模型请求示例', modelDescription: '示例使用占位模型 ID；生产环境请以 /v1/models 返回的 ID 替换。', videoModelDescription: '视频模型使用异步创建、轮询和内容下载流程。', imageModelDescription: '图片模型使用 OpenAI Images 生成或编辑流程。', poll: '查询任务并下载视频',
        },
