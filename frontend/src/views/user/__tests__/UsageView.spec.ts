...
const messages: Record<string, string> = {
  ...
  'usage.tokenIncentive.title': 'Token incentive',
  'usage.tokenIncentive.description': 'Weekly reward',
  'usage.tokenIncentive.expireHint': 'Claim this week',
  'usage.tokenIncentive.used': 'Used',
  'usage.tokenIncentive.target': 'Target',
  'usage.tokenIncentive.reward': 'Reward',
  'usage.tokenIncentive.week': 'Week',
  'usage.tokenIncentive.progress': 'Progress',
  'usage.tokenIncentive.eligible': 'Eligible',
  'usage.tokenIncentive.claimed': 'Claimed',
  'usage.tokenIncentive.inProgress': 'In progress',
  'usage.tokenIncentive.claim': 'Claim',
  'usage.tokenIncentive.claiming': 'Claiming',
  'usage.tokenIncentive.claimSuccess': 'Claim success',
  'usage.tokenIncentive.currentTier': 'Current tier {reward}',
  'usage.tokenIncentive.claimedAt': 'Claimed at {time}',
  'usage.tokenIncentive.remaining': 'Remaining {tokens}',
  'usage.tokenIncentive.loadFailed': 'Load incentive failed',
  'usage.tokenIncentive.disabled': 'Disabled',
  'usage.tokenIncentive.notEligible': 'Not eligible',
  'usage.tokenIncentive.alreadyClaimed': 'Already claimed',
  'usage.tokenIncentive.claimFailed': 'Claim failed',
...
const tokenIncentiveStatus = {
  enabled: true,
  eligible: true,
  claimable: true,
  claimed: false,
  week_start: '2026-06-29T00:00:00+08:00',
  week_end: '2026-07-06T00:00:00+08:00',
  tokens: 120_000_000,
  threshold_tokens: 100_000_000,
  reward_amount: 5,
  claimed_reward_amount: 2,
  claimed_threshold_tokens: [50_000_000],
  rules: [
  ...
  next_threshold_tokens: 500_000_000,
  next_reward_amount: 10,
  claimed_at: null,
...
describe('user UsageView', () => {
  beforeEach(() => {
    ...
    claimTokenIncentive.mockResolvedValue(tokenIncentiveStatus)
    refreshUser.mockResolvedValue(undefined)
  })

  it('loads logs, stats, model stats, snapshot, and token incentive status on first render', async () => {
    mountUsageView()
    await flushPromises()
    ...
    expect(getTokenIncentiveStatus).toHaveBeenCalled()
  })

  it('shows token incentive progress and claims the current reached tier', async () => {
    getTokenIncentiveStatus.mockResolvedValue(tokenIncentiveStatus)
    claimTokenIncentive.mockResolvedValue({
      ...tokenIncentiveStatus,
      claimable: false,
      claimed: true,
      claimed_reward_amount: 7,
      claimed_threshold_tokens: [50_000_000, 100_000_000],
      claimed_at: '2026-06-30T12:00:00+08:00',
    })

    const wrapper = mountUsageView()
    await flushPromises()

    expect(wrapper.text()).toContain('Token incentive')
    expect(wrapper.text()).toContain('120.00M')
    ...
    expect(claimTokenIncentive).toHaveBeenCalledTimes(1)
    expect(refreshUser).toHaveBeenCalledTimes(1)
    expect(showSuccess).toHaveBeenCalledWith('Claim success')
  })
