...
import { keysAPI, usageAPI, userAPI, userGroupsAPI } from '@/api'
...
const tokenIncentiveProgressText = computed(() => {
  const status = tokenIncentive.value
  if (!status) return ''
  return `${formatTokens(status.tokens)} / ${formatTokens(tokenIncentiveMaxThreshold.value)}`
})

const tokenIncentiveRemainingTokens = computed(() => {
  const status = tokenIncentive.value
  if (!status) return 0
  const target = status.next_threshold_tokens || status.threshold_tokens
  return Math.max(target - status.tokens, 0)
})
