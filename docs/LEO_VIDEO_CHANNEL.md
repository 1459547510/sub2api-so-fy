...
任务记录和结算标记持久化在 `video_jobs` 表，本地文件路径由 `job_id` 确定，Sub2API 重启后会继续保存和结算，不依赖浏览器在线。同步兼容接口也会拒绝 HTTP 成功但没有有效视频 URL 的上游响应，因此不会为这类空成功响应记录用量；同步接口保留标准媒体字段，但会过滤 provider、UUID、generation_id、account_id、source_url 和 video_url 等内部元数据，平台工作台使用上述异步本地成品链路。
