...
用户工作台提交单张图片时使用兼容性已验证的 `image_url` 首帧链路；提交 2 到 4 张图片时使用 `guidances.image_reference`，并按数组位置显式写入从 `0` 开始的连续 `order`。直接调用 API 时可以省略非负整数 `order`，最新版 LeoStudio 会按引用顺序补齐未指定项；外部 API 仍兼容 `image_urls`，但调用方应优先使用上述两种明确语义。
